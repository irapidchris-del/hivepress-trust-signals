...
function hpts_init() {
	...
	// Hook signatures (verified in core Hook component): {model}/create fires
	// with ( $id, $object ); {model}/update_status for post models fires with
	// ( $id, $new_status, $old_status, $object ).
	add_action( 'hivepress/v1/models/message/create', 'hpts_on_message_create', 10, 2 );
	...
	add_action( 'hivepress/v1/models/message/create', 'hpts_track_message_activity', 10, 2 );
...
function hpts_admin_scripts() {
	...
	wp_add_inline_script( 'wp-color-picker', 'jQuery(function($){$(".hpts-color-field").wpColorPicker();});' );
}
			...
			'signals' => [
				...
				'fields' => [
					'trust_signals_items'       => [
						...
						'description' => __( 'Signals that depend on an extension are hidden automatically if that extension is inactive or there is not enough data. Response time is hidden above 3 days; response rate is shown only at 80% or higher. Signals are omitted rather than estimated.', 'hivepress-trust-signals' ),
						'type'        => 'checkboxes',
 ...
 */
function hpts_get_option( $name, $default ) {
...
function hpts_collect_signals( $vendor_id, $listing_id = 0 ) {
	...
	if ( in_array( 'verified', $enabled, true ) ) {
		...
		if ( ! $verified ) {
			$debug[] = sprintf(
				'verified: hidden (hp_verified meta - vendor #%d: %s%s). Tick "Verified" on the vendor in WP admin to enable.',
				$vendor_id,
				$vendor_verified ? 'yes' : 'empty',
				$listing_id ? sprintf( '; listing #%d: %s', $listing_id, $listing_verified ? 'yes' : 'empty' ) : ''
			);
		} else {
			$signals[] = [
				'key'   => 'verified',
	...
	if ( in_array( 'completed_bookings', $enabled, true ) ) {
		if ( ! isset( $cached['bookings'] ) || null === $cached['bookings'] ) {
	...
	if ( in_array( 'completed_bookings', $enabled, true ) && isset( $cached['bookings'] ) && null !== $cached['bookings'] && $cached['bookings'] > 0 ) {
		$signals[] = [
	...
	if ( $response && $response['samples'] >= $min_samples ) {
		...
		if ( in_array( 'response_time', $enabled, true ) && null !== $response['median'] ) {
			...
			if ( ! $bucket ) {
				$debug[] = 'response_time: hidden (median response is slower than 3 days; omitted by design).';
			} else {
				$signals[] = [
					'key'   => 'response_time',
					'icon'  => 'clock',
		...
		if ( in_array( 'response_rate', $enabled, true ) && null !== $response['rate'] ) {
			if ( $response['rate'] < 80 ) {
				$debug[] = sprintf( 'response_rate: hidden (%d%%; shown only at 80%%+ by design).', $response['rate'] );
			} else {
				$signals[] = [
					'key'   => 'response_rate',
					'icon'  => 'reply',
...
function hpts_response_bucket( $seconds ) {
	if ( $seconds < HOUR_IN_SECONDS ) {
		return __( 'within an hour', 'hivepress-trust-signals' );
	}

	if ( $seconds < 6 * HOUR_IN_SECONDS ) {
		return __( 'within a few hours', 'hivepress-trust-signals' );
	}

	if ( $seconds < DAY_IN_SECONDS ) {
		return __( 'within a day', 'hivepress-trust-signals' );
	}

	if ( $seconds <= 3 * DAY_IN_SECONDS ) {
		return __( 'within a few days', 'hivepress-trust-signals' );
	}

	// Slower than 3 days: omit rather than advertise.
	return null;
}
...
function hpts_get_vendor_stats( $vendor_id ) {
	...
	if ( is_array( $stats ) && isset( $stats['v'] ) && HPTS_VERSION === $stats['v'] ) {
		return $stats;
	}
...
function hpts_compute_response_stats( $vendor_user_id ) {
	...
	if ( ! $rows ) {
		return [ 'median' => null, 'rate' => null, 'samples' => 0 ];
	}
	...
	$rate_total  = 0;
	$rate_hits   = 0;
	$grace_limit = time() - 2 * DAY_IN_SECONDS;
...
function hpts_on_message_create( $message_id, $message ) {
	if ( ! is_object( $message ) ) {
		return;
	}

	foreach ( [ 'get_sender__id', 'get_recipient__id' ] as $method ) {
		if ( method_exists( $message, $method ) ) {
			hpts_flush_vendor_stats( hpts_vendor_id_from_user( (int) call_user_func( [ $message, $method ] ) ) );
		}
	}
}
...
function hpts_track_message_activity( $message_id, $message ) {
	if ( is_object( $message ) && method_exists( $message, 'get_sender__id' ) ) {
		$sender_id = (int) $message->get_sender__id();

		if ( $sender_id ) {
			update_user_meta( $sender_id, 'hpts_last_active', time() );
		}
	}
}
