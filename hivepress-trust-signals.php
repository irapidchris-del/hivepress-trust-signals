...
function hpts_maybe_upgrade() {
	...
	// 1.7.1 split the single sidebar-order option into separate listing and
	// vendor options. Copy any existing combined value onto both so upgraded
	// sites keep their placement, then drop the old option.
	$legacy_order = get_option( 'hp_trust_signals_order', null );

	if ( null !== $legacy_order && false !== $legacy_order ) {
		if ( false === get_option( 'hp_trust_signals_order_listing', false ) ) {
			update_option( 'hp_trust_signals_order_listing', $legacy_order );
		}

		if ( false === get_option( 'hp_trust_signals_order_vendor', false ) ) {
			update_option( 'hp_trust_signals_order_vendor', $legacy_order );
		}

		delete_option( 'hp_trust_signals_order' );
	}
 ...
 */
define( 'HPTS_UPDATE_REASON_KEY', 'hpts_github_release_reason' );
 ...
 */
define( 'HPTS_UPDATE_RATE_LIMIT_KEY', 'hpts_github_release_rate_limit' );
 ...
 * @return void
 */
function hpts_schedule_release_refresh() {
	$hook = HPTS_UPDATE_CACHE_KEY . '_refresh';
 ...
 * @return void
 */
function hpts_refresh_release() {
	hpts_get_latest_release( true );
}

add_action( HPTS_UPDATE_CACHE_KEY . '_refresh', 'hpts_refresh_release' );
...
function hpts_get_latest_release( $force = false ) {
	$cached = get_site_transient( HPTS_UPDATE_CACHE_KEY );
	...
	if ( ! $release && $cached ) {
		set_site_transient( HPTS_UPDATE_CACHE_KEY, $cached, HOUR_IN_SECONDS );
 ...
 */
function hpts_fetch_release_data() {
	$site = hpts_fetch_release_from_site();

	if ( isset( $site['release'] ) ) {
		delete_site_transient( HPTS_UPDATE_REASON_KEY );
	...
	if ( isset( $site['reason'] ) && 'no_release' === $site['reason'] ) {
		set_site_transient( HPTS_UPDATE_REASON_KEY, 'no_release', HOUR_IN_SECONDS );

		return null;
	}

	return hpts_fetch_release_from_api();
}
 ...
 *                              back to the API.
 */
function hpts_fetch_release_from_site() {
	$base = 'https://github.com/' . HPTS_UPDATE_REPO;

	$response = hpts_request(
	...
	$asset = hpts_fetch_release_asset( $base, $tag );
	...
	$notes = hpts_fetch_release_notes( $base, $tag );
 ...
 * @return array<string, string>|null
 */
function hpts_fetch_release_asset( $base, $tag ) {
	$response = hpts_request( $base . '/releases/expanded_assets/' . rawurlencode( $tag ) );
 ...
 * @return array<string, string>
 */
function hpts_fetch_release_notes( $base, $tag ) {
	$empty = [
		'body'      => '',
		'published' => '',
	];

	$response = hpts_request( $base . '/releases.atom' );
	...
	foreach ( $entries[1] as $entry ) {
		...
		if ( preg_match( '#<content[^>]*>(.*?)</content>#s', $entry, $content ) ) {
			$notes = hpts_release_notes_to_text( $content[1] );
		}
 ...
 * @return string
 */
function hpts_release_notes_to_text( $html ) {
 ...
 *
 * Kept as a fallback only. See `hpts_fetch_release_data()` for why it is not the first choice.
 *
 * @return array<string, mixed>|null
 */
function hpts_fetch_release_from_api() {
	...
	if ( get_site_transient( HPTS_UPDATE_RATE_LIMIT_KEY ) ) {
		set_site_transient( HPTS_UPDATE_REASON_KEY, 'rate_limited', HOUR_IN_SECONDS );
	...
	$response = wp_remote_get(
		...
		[
			...
			'user-agent' => 'hivepress-trust-signals/' . HPTS_VERSION,
		]
	);

	if ( is_wp_error( $response ) ) {
		set_site_transient( HPTS_UPDATE_REASON_KEY, 'unreachable', HOUR_IN_SECONDS );
	...
	if ( 200 !== $code ) {
		...
		if ( ( 403 === $code || 429 === $code ) && '0' === (string) wp_remote_retrieve_header( $response, 'x-ratelimit-remaining' ) ) {
			...
			set_site_transient( HPTS_UPDATE_RATE_LIMIT_KEY, $reset ? $reset : time() + $wait, $wait );
		}

		set_site_transient( HPTS_UPDATE_REASON_KEY, $reason, HOUR_IN_SECONDS );

		return null;
	}

	delete_site_transient( HPTS_UPDATE_RATE_LIMIT_KEY );
	delete_site_transient( HPTS_UPDATE_REASON_KEY );
 ...
 * @return array<string, mixed>|null
 */
function hpts_request( $url, $args = [] ) {
	$response = wp_remote_get(
		$url,
		array_merge(
			[
...
function hpts_handle_update_check() {
	...
	$reason = get_site_transient( HPTS_UPDATE_REASON_KEY );
	...
	if ( 'no_release' === $reason ) {
		...
		$status = 'error';
	} elseif ( $release && version_compare( $release['version'], hpts_update_version(), '>' ) ) {
		$status = 'available';
	} else {
		$status = 'none';
	}
...
function hpts_update_check_notice() {
	...
	if ( 'available' === $status ) {
		...
		$message = __( 'No releases have been published for Trust Signals for HivePress yet, so there is nothing to update to. This is normal for a brand new copy and does not mean anything is wrong.', 'hivepress-trust-signals' );
		$class   = 'notice-info';
	} elseif ( 'limited' === $status ) {
		$message = __( 'GitHub limits how many update checks one server may make each hour, and this server has reached that limit. Nothing is wrong with the plugin or your site, and checking will work again within the hour.', 'hivepress-trust-signals' );
